    ...
    return (
        <div className='flex flex-row gap-1 items-center'>
                <svg 
                    ...
                    onClick={() => onSubmitEmail(prompt("Put your email here if you want to be notified when I post a new Peace Corps blog in the future! Note: this action might take a little bit if my backend needs to cold boot"))} 
                ...
                {!isLoading ? <p className={twMerge('flex flex-row items-center transition-all w-0 h-8 overflow-hidden text-nowrap', overNotifications && 'w-48')}>Get notified of new blogs!</p> : <img src={logo} alt="JN logo" className="animate-spin w-4" />}
        </div>
    );
}