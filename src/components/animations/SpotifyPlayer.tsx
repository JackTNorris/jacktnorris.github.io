        ...
        <div className="md:flex flex-col gap-3 w-36 hidden md:visible items-start">
            ...
            <div className="h-24 w-24">
                    <div className={twMerge('transition-all absolute flex flex-row items-center', hoverRecord && 'ml-14')}>
                        ...
                        <p className={twMerge("h-20 text-wrap text-center text-xs opacity-0", hoverRecord && 'opacity-100')} style={{textOrientation: 'mixed', writingMode: 'vertical-rl'}}>{currentRecord.name}</p>
                    </div>
                    <img onClick={() => window.open(currentRecord.link)} onMouseLeave={() => setHoverRecord(false)} onMouseEnter={() => setHoverRecord(true)} className="transition-all absolute h-24 w-24 rounded-lg shadow-2xl shadow-blue-300 hover:scale-105 hover:cursor-pointer" src={currentRecord?.image} />
            </div>
        </div>
    ) : <></>
}