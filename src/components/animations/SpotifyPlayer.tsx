    ...
    return (
        <div className="md:flex flex-col gap-3 w-36 hidden md:visible items-start">
            ...
            <div className="h-24 w-24">
                    ...
                    <ImageLoader onClick={() => currentRecord && window.open(currentRecord.link)} onMouseLeave={() => setHoverRecord(false)} onMouseEnter={() => setHoverRecord(true)} className="transition-all absolute h-24 w-24 rounded-lg shadow-2xl shadow-blue-300 hover:scale-105 hover:cursor-pointer border border-black" src={currentRecord?.image || ''} />
            </div>
        </div>
    );
}